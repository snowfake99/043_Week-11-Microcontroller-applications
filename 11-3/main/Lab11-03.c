...
#include "driver/adc.h"          // legacy ADC API
...
void app_main(void)
{
    ...
    // กำหนดค่า ADC (12 บิต + attenuation 12 dB)
    ...
    while (1) {
        ...
        float rawVoltage         = raw_voltage_mv / 1000.0f;
        float oversampledVoltage = oversampled_voltage_mv / 1000.0f;
        float filteredVoltage    = filtered_voltage_mv / 1000.0f;

        ESP_LOGI(TAG, "=== การเปรียบเทียบ ===");
        ESP_LOGI(TAG, "Raw ADC: %u (%.3fV)", rawValue, rawVoltage);
