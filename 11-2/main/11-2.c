...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(adc1_config_channel_atten(LDR_CHANNEL, ADC_ATTEN_DB_11));

    // ปรับเทียบ ADC
    adc_chars = calloc(1, sizeof(esp_adc_cal_characteristics_t));
    if (adc_chars == NULL) {
        ESP_LOGE(TAG, "หน่วยความจำไม่พอสำหรับ adc_chars");
        return;
    }
    ...
    while (1) {
        ...
        // แปลงเป็นระดับแสง (%) แบบสมมติ
        float lightLevel = (adc_reading / 4095.0) * 100.0;

        // ส่งข้อมูลออก Serial (3 ค่า: ADC, Voltage, Light%)
        // ใช้ \t (Tab) คั่น เพื่อให้ Serial Plotter แสดงได้หลายเส้น
        printf("ADC: %4lu | Voltage: %5.2f V | Light: %5.1f %%\n", adc_reading, voltage, lightLevel);
